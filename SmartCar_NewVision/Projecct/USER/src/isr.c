...
{       
   ...
   if(!gpio_get(D2)){
     ...
     else if(!Info.dip[1] && Info.dip[2])Parameter.ExpectSpeed-=5;
     ...
     if(Info.dip[1] && Info.dip[2])Parameter.P_Balance-=1;
...
{
    CLEAN_KBI1_FLAG;
    if(gpio_get(F3))Parameter.ExpectSpeed=280,gpio_turn(D1);           //C F3//270
