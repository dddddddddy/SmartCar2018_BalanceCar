...
int16 GetOffset(){
 ...
 // static uint8 dir=0;
  ...
  for(i=0;i<5;i++){
    sum[0]+=adc_once(ADC0_SE11,ADC_12bit);  //C3     
    sum[1]+=adc_once(ADC0_SE10,ADC_12bit);  //C2  
    sum[2]+=adc_once(ADC0_SE9,ADC_12bit);   //C1
    sum[3]+=adc_once(ADC0_SE8,ADC_12bit);   //C0
    sum[4]+=adc_once(ADC0_SE15,ADC_12bit);  //F6//1
  }
  
  Info.AD[2]=SlidingFilter(Info.buff[0],SlidingFilterWidth,sum[0]/5);
  ...
  Info.AD[4]=SlidingFilter(Info.buff[4],SlidingFilterWidth,sum[4]/5);
 ...
 Info.AD[2]=Info.AD[2]<100?100:Info.AD[2];
 Info.AD[4]=Info.AD[4]<100?100:Info.AD[4];
   uint32 sumad=0;
  sumad=Info.AD[1]+Info.AD[5];
 if(sumad<10)Flag.FlagMotor=-1;
  /*****get offset***********/
  ...
  offsetmiddle=((int16)(submiddle*200.0f/addmiddle)-TurnOffset);
 ...
 if(Flag.time>=1){
   Flag.Circle=0;
  Flag.time++;
  if(Flag.time>200)Flag.time=0;
 }
  if((Info.AD[1]>2500||Info.AD[5]>2500) && Flag.Circle==0 && !Flag.delay){ 
    Flag.Circle=-1;
  }
  if(Flag.Circle==-1){
...
void ShowMenu(){
  ...
  OLED_P6x8Str(54,2,"ES");OLED_Print_Num1(66,2,Parameter.ExpectSpeed);
  ...
  OLED_Print_Num1(0,5,Info.AD[1]);OLED_Print_Num1(36,5,Info.AD[5]);
  OLED_Print_Num1(0,6,Info.AD[3]);
  ...
  OLED_P6x8Str(0,0,"OF");OLED_Print_Num1(12,0,Info.offset);
  //OLED_P6x8Str(54,0,"P");OLED_Print_Num1(66,0,Parameter.D_Balance);
